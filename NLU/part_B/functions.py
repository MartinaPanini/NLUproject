...
def train_loop(data, optimizer, criterion_slots, criterion_intents, model, clip=5):
    """
    Funzione di training. Per ogni campione nel dataset, calcola la loss totale
    (somma della loss per gli intenti e quella per gli slot) e aggiorna i pesi del modello.
    """
    model.train()
    loss_array = []
...
def eval_loop(data, criterion_slots, criterion_intents, model, lang):
    """
    Funzione di valutazione. Calcola la loss e genera il report per intenti e slot.
    """
    model.eval()
    loss_array = []
